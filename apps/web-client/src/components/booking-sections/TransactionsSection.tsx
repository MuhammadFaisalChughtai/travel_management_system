...
export function TransactionsSection({
  ...
  >("All");
  const [customMarginPercentage, setCustomMarginPercentage] =
    useState<string>("0");
  ...
  const clientPayments =
    booking.payments
      ?.filter(
        (p) =>
          p.paymentType === "Received from Client" &&
  ...
  const flightCost =
    booking.flightServices?.reduce(
      (sum, f) => sum + (parseFloat(f.price) || 0),
      0,
    ) || 0;
  const accCost =
  ...
  const addCost =
    booking.additionalServices?.reduce(
      (sum, s) => sum + (parseFloat(s.charges) || 0),
      0,
    ) || 0;
  ...
  const totalDiscounts =
    booking.discounts?.reduce(
      (sum, d) => sum + (parseFloat(d.amount) || 0),
      0,
    ) || 0;
  const refundsToClient =
    booking.refunds
      ?.filter((r) => r.direction === "Refund to Client")
      .reduce((sum, r) => sum + (parseFloat(r.amount) || 0), 0) || 0;
  const refundsFromVendor =
    booking.refunds
      ?.filter((r) => r.direction === "Refund from Vendor")
      .reduce((sum, r) => sum + (parseFloat(r.amount) || 0), 0) || 0;
  ...
  const marginPaidToAgent =
    booking.payments
      ?.filter((p) => p.paymentType === "Margin Paid to Agent")
      .reduce((sum, p) => sum + (parseFloat(p.amount) || 0), 0) || 0;
  const creditCardCharges =
    booking.payments
      ?.filter((p) => p.paymentType === "Credit Card Charges")
      .reduce((sum, p) => sum + (parseFloat(p.amount) || 0), 0) || 0;
    ...
    const fetchAgentMargin = async () => {
      ...
      try {
        const res = await api.get(
          `/agents/by-name/${encodeURIComponent(booking.agentName!)}`,
        );
  ...
  const totalAgentMargin =
    ((netProfit + marginPaidToAgent) * marginPercentage) / 100;
  const remainingAgentMargin = Math.max(
    0,
    totalAgentMargin - marginPaidToAgent,
  );

  // Gross margin % (profit as a % of net total received -- shown as informational)
  const grossMarginPct =
    clientPayments - refundsToClient > 0
      ? (netProfit / (clientPayments - refundsToClient)) * 100
      : 0;
  ...
  const filteredTransactions = allCombinedTransactions.filter((p) => {
    ...
    if (p._type === "payment" || p._type === "vendorPayment")
      return (p as any).paymentType === filter;
  ...
  filteredTransactions.sort(
    (a, b) =>
      new Date(b.paidOn || new Date()).getTime() -
      new Date(a.paidOn || new Date()).getTime(),
  );
    ...
    <div className="space-y-6">
        ...
        <div className="relative z-10 p-4">
          <div className="flex justify-between items-start mb-4">
            <div>
              ...
              <p className="text-indigo-200 text-[11px] uppercase tracking-wide font-bold mt-1">
                Real-time Booking Profitability
              </p>
            </div>
            <div className="flex gap-2">
              {onAddDiscount && (
                <button
                  ...
                  className="bg-white/10 hover:bg-white/20 text-white px-3 py-1.5 rounded-lg text-[10px] font-bold transition-all border border-white/10 flex items-center gap-1.5"
                >
              ...
              {onLogRefund && (
                <button
                  onClick={() => setShowLogRefundModal(true)}
                  className="bg-white/10 hover:bg-white/20 text-white px-3 py-1.5 rounded-lg text-[10px] font-bold transition-all border border-white/10 flex items-center gap-1.5"
                >
              ...
              {onFinalizeMargin &&
                ...
                booking.marginStatus !== "Paid" && (
                  <button
                    onClick={() =>
                      onFinalizeMargin({
                        amount: remainingAgentMargin.toString(),
                        notes: "",
                      })
                    }
              ...
              {onClawbackMargin && (
                <button
                  ...
                  className="bg-purple-500/20 hover:bg-purple-500/40 text-purple-100 px-3 py-1.5 rounded-lg text-[10px] font-bold transition-all border border-purple-500/30 flex items-center gap-1.5"
                >
          ...
          {isOverpaid && (
            <div className="mb-4 p-3 bg-amber-500/10 border border-amber-500/30 rounded-xl text-amber-200 text-xs font-semibold flex items-center gap-2">
              ...
              <span>
                Unallocated funds detected. Please either Log a Refund or Update
                the Invoice Price to proceed.
              </span>
            </div>
          )}
          ...
          <div className="grid grid-cols-2 md:grid-cols-9 gap-3">
            ...
            >
              <p className="text-[9px] text-indigo-200 font-bold uppercase mb-1 flex items-center justify-between">
                ...
                {onUpdateInvoicePrice && (
                  <Edit2 className="w-3 h-3 text-indigo-300 opacity-60 group-hover:opacity-100 transition-opacity" />
                )}
              </p>
              <p className="font-black text-white text-lg">
                {symbol}
                {bookingTotal.toLocaleString(undefined, {
            ...
            >
              <p className="text-[9px] text-indigo-200 font-bold uppercase mb-1 flex items-center justify-between">
                Total Received{" "}
                <ArrowDownLeft className="w-3 h-3 text-emerald-400 opacity-50" />
              </p>
              <p className="font-black text-emerald-400 text-lg">
                {symbol}
                {(clientPayments - refundsToClient).toLocaleString(undefined, {
                  minimumFractionDigits: 2,
                })}
              </p>
            </div>
            ...
            >
              <p className="text-[9px] text-indigo-200 font-bold uppercase mb-1 flex items-center justify-between">
                Total Refunded{" "}
                <RefreshCcw className="w-3 h-3 text-rose-400 opacity-50" />
              </p>
              <p className="font-black text-rose-400 text-lg">
                {symbol}
                {refundsToClient.toLocaleString(undefined, {
                  minimumFractionDigits: 2,
                })}
              </p>
            </div>
            ...
            <div className="bg-white/5 rounded-xl p-3 border border-white/10">
              ...
              <p
                className={`font-black text-lg ${clientBalance > 0 ? "text-amber-400" : clientBalance < 0 ? "text-rose-400" : "text-slate-300"}`}
              >
                {symbol}
                {Math.abs(clientBalance).toLocaleString(undefined, {
                  minimumFractionDigits: 2,
                })}
              </p>
              {clientBalance === 0 ? (
                <p className="text-[8px] text-emerald-300 mt-0.5 font-bold">
                  Fully Paid
                </p>
              ) : clientBalance < 0 ? (
                <p className="text-[8px] text-rose-300 mt-0.5 font-bold">
                  Overpaid
                </p>
              ) : (
                <p className="text-[8px] text-amber-300/70 mt-0.5">
                  Pending Payment
                </p>
              )}
            </div>
            ...
            >
              <p className="text-[9px] text-indigo-200 font-bold uppercase mb-1 flex items-center justify-between">
                Total Sent{" "}
                <ArrowUpRight className="w-3 h-3 text-red-400 opacity-50" />
              </p>
              <p className="font-black text-red-400 text-lg">
                {symbol}
                {netSent.toLocaleString(undefined, {
                  minimumFractionDigits: 2,
                })}
              </p>
              {refundsFromVendor > 0 && (
                <p className="text-[8px] text-red-300 mt-0.5">
                  After {symbol}
                  {refundsFromVendor.toFixed(2)} refunded by vendor
                </p>
              )}
            </div>
            ...
            <div className="bg-white/5 rounded-xl p-3 border border-white/10">
              ...
              <p
                className={`font-black text-lg ${remainingVendorPay > 0 ? "text-amber-400" : "text-slate-300"}`}
              >
                {symbol}
                {remainingVendorPay.toLocaleString(undefined, {
                  minimumFractionDigits: 2,
                })}
              </p>
            ...
            <div className="bg-white/5 rounded-xl p-3 border border-white/10">
              ...
              <p className="font-black text-amber-400 text-lg">
                {symbol}
                {totalDiscounts.toLocaleString(undefined, {
                  minimumFractionDigits: 2,
                })}
              </p>
            </div>
              ...
              !loadingMargin ? (
                <div className="flex items-center gap-1.5">
                  <span className="font-black text-blue-400 text-lg">
                    {symbol}
                    {remainingAgentMargin.toLocaleString(undefined, {
                      minimumFractionDigits: 2,
                    })}
                  </span>
                  <input
                    ...
                    className="w-10 bg-white/10 text-white text-[9px] px-1 py-0.5 rounded border border-white/20 outline-none"
                    placeholder="%"
                  />
                </div>
              ) : (
                <div className="font-black text-blue-400 text-lg">
                  {loadingMargin ? (
                    <div className="w-4 h-4 mt-1 border-2 border-blue-400/30 border-t-blue-400 rounded-full animate-spin" />
                  ) : (
                    `${symbol}${remainingAgentMargin.toLocaleString(undefined, { minimumFractionDigits: 2 })}`
                  )}
                </div>
              )}
              <p className="text-[8px] text-indigo-300 mt-0.5 italic">
                Remaining to pay
              </p>
              {marginPaidToAgent > 0 && (
                <p className="text-[10px] text-emerald-400 font-bold mt-1">
                  Paid: {symbol}
                  {marginPaidToAgent.toLocaleString(undefined, {
                    minimumFractionDigits: 2,
                  })}
                </p>
              )}
            ...
            >
              <p className="text-[9px] text-emerald-100 font-bold uppercase mb-1 flex items-center justify-between">
                Net Profit{" "}
                <PieChart className="w-3 h-3 text-emerald-100 opacity-50" />
              </p>
              <p
                className={`font-black text-xl ${netProfit >= 0 ? "text-white" : "text-red-300"}`}
              >
                {netProfit < 0 ? "-" : ""}
                {symbol}
      ...
      {hasPendingVendorPayments && (
        <div className="bg-amber-50 border border-amber-200 rounded-xl p-4 flex items-start gap-3 shadow-sm">
          ...
          <div>
            <h4 className="text-[12px] font-bold text-amber-800">
              Pending Vendor Payments
            </h4>
            <p className="text-[11px] text-amber-700 mt-0.5">
              There are services added to this booking, but no vendor payments
              have been logged yet. Please ensure vendors are paid to secure the
              bookings.
            </p>
          </div>
        </div>
                    ...
                    <table className="w-full text-left border-collapse whitespace-nowrap">
                      <thead>
                        <tr className="border-b border-slate-200 text-[10px] uppercase text-slate-400 bg-slate-50/50">
                          <th className="py-2 px-4 font-bold rounded-tl-xl">
                            Date
                          </th>
                          <th className="py-2 px-4 font-bold">Type</th>
                          <th className="py-2 px-4 font-bold w-full">
                            Details
                          </th>
                          <th className="py-2 px-4 font-bold text-right rounded-tr-xl">
                            Amount
                          </th>
                        </tr>
                      </thead>
                      <tbody className="text-[12px]">
                        {paginatedTxs.map((item: any) => {
                          if (
                            item._type === "payment" ||
                            item._type === "vendorPayment"
                          ) {
                            const t = item;
                            const isReceived =
                              t.paymentType === "Received from Client";
                              ...
                              >
                                ...
                                <td className="py-2.5 px-4 font-bold text-slate-800">
                                  <div className="flex items-center gap-2">
                                    ...
                                    <div className="flex flex-col">
                                      ...
                                      {/* Status Badge */}
                                      {isPending && (
                                        <span className="text-[9px] font-extrabold text-amber-700 bg-amber-100 px-1.5 py-0.5 rounded-full w-fit tracking-wide">
                                          ⏳ PENDING APPROVAL
                                        </span>
                                      )}
                                      {isRejected && (
                                        <span className="text-[9px] font-extrabold text-red-600 bg-red-100 px-1.5 py-0.5 rounded-full w-fit tracking-wide">
                                          ✗ REJECTED
                                        </span>
                                      )}
                                    </div>
                                ...
                                <td className="py-2.5 px-4">
                                  <div className="flex flex-col gap-0.5">
                                    <span
                                      className={`font-medium ${isRejected ? "text-slate-400" : "text-slate-600"}`}
                                    >
                                      {t.paymentMethod}
                                    </span>
                                    {t.loggedByName && (
                                    ...
                                    {t.notes && (
                                      <span className="text-[10px] text-slate-400 italic whitespace-normal">
                                        "{formatPendingNotes(t.notes)}"
                                      </span>
                                    )}
                                    {t.evidenceUrl && (
                                      <a
                                        ...
                                        target="_blank"
                                        rel="noreferrer"
                                        className="flex items-center gap-1 text-[10px] text-indigo-600 hover:text-indigo-800 font-semibold w-fit"
                                      >
                          ...
                          if (item._type === "refund") {
                            const r = item;
                            const isFromVendor =
                              r.direction === "Refund from Vendor";
                            return (
                              <tr
                                key={`ref-${r.id}`}
                              ...
                              >
                                ...
                                <td className="py-2 px-4 font-bold text-rose-800">
                                  <div className="flex items-center gap-2">
                                    <div
                                      className={`w-6 h-6 rounded-full flex items-center justify-center ${isFromVendor ? "bg-emerald-100 text-emerald-600" : "bg-red-100 text-red-600"}`}
                                    >
                                ...
                                <td className="py-2 px-4">
                                  <div className="flex flex-col">
                                    ...
                                    {r.notes && (
                                      <span className="text-[10px] text-rose-400 italic whitespace-normal">
                                        "{r.notes}"
                                      </span>
                                    )}
                                  </div>
                                ...
                                <td
                                  className={`py-2 px-4 text-right font-black ${isFromVendor ? "text-emerald-600" : "text-red-600"}`}
                                >
                                  {isFromVendor ? "+" : "-"}
                                  {symbol}
                              ...
                              >
                                ...
                                <td className="py-2 px-4">
                                  <div className="flex flex-col">
                                    ...
                                    {d.notes && (
                                      <span className="text-[10px] text-amber-600 italic whitespace-normal">
                                        "{d.notes}"
                                      </span>
                                    )}
                                  </div>
