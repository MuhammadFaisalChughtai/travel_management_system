  ...
  return (
    <div className="w-full bg-white border border-slate-200 focus-within:border-primary-500 focus-within:ring-4 focus-within:ring-primary-500/10 rounded-xl px-3 py-1.5 transition-all flex flex-wrap gap-2 items-center min-h-[38px]">
      {tags.map((tag, i) => (
        <span
          key={i}
          className="flex items-center gap-1 bg-primary-100 text-primary-700 px-2 py-0.5 rounded text-[11px] font-bold tracking-wide"
        >
          {tag}
          <button
            type="button"
...
function AgentDetailsModal({
  ...
  useEffect(() => {
    if (isOpen && initialAgent) {
      ...
      setEditMode(false);
      const initialClientTags = initialAgent.client
        ? initialAgent.client
            .split(",")
            .map((s) => s.trim())
            .filter(Boolean)
      ...
      setSegments(
        initialAgent.marginSegments.length > 0
          ...
          : DEFAULT_SEGMENTS.map((s) => ({
              ...s,
              label: s.label.replace(/£/g, symbol),
            })),
      );
    }
  ...
  const handleSaveSegments = async () => {
    ...
    try {
      ...
      const res = await api.post(`/agents/${agent.id}/margin-segments`, {
        segments: payload,
      });
      setAgent(res.data.agent);
  ...
  const addSegment = () => {
    setSegments([
      ...segments,
      { minAmount: "", maxAmount: "", marginPercent: "", label: "" },
    ]);
  };
  ...
  const infoRows = [
    ...
    {
      label: "Created At",
      value: new Date(agent.createdAt).toLocaleString("en-GB", {
        dateStyle: "medium",
        timeStyle: "short",
      }),
    },
    {
      label: "Updated At",
      value: new Date(agent.updatedAt).toLocaleString("en-GB", {
        dateStyle: "medium",
        timeStyle: "short",
      }),
    },
              ...
              {editMode ? (
                <>
                  <button
                    ...
                    className="px-5 py-2 rounded-xl bg-white/10 hover:bg-white/20 border border-white/20 text-white text-[12px] font-bold transition-all backdrop-blur-sm shadow-sm"
                  >
                    Cancel
                  </button>
                  <button
                    ...
                    disabled={saving}
                    className="px-5 py-2 rounded-xl bg-white text-primary-900 hover:bg-slate-50 text-[12px] font-bold transition-all flex items-center gap-2 shadow-lg shadow-white/20 active:scale-95"
                  >
                    {saving ? (
                ...
                <button
                  ...
                  className="px-5 py-2 rounded-xl bg-white/10 hover:bg-white/20 border border-white/20 text-white text-[12px] font-bold transition-all flex items-center gap-2 backdrop-blur-sm shadow-sm"
                >
              ...
              {editMode ? (
                <div className="p-2 grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div>
                    ...
                    <input
                      ...
                      className="w-full bg-white/70 border border-slate-200 focus:border-primary-500 focus:ring-4 focus:ring-primary-500/10 rounded-xl px-4 py-2.5 text-slate-800 text-[13px] outline-none transition-all shadow-sm"
                    />
                  </div>
                  <div>
                    <label className="block text-[11px] font-bold text-slate-500 uppercase tracking-wider mb-2">
                  ...
                  <div>
                    ...
                    <input
                      type="text"
                      value={form.gdsSystem}
                      onChange={(e) =>
                        setForm({ ...form, gdsSystem: e.target.value })
                      }
                      className="w-full bg-white/70 border border-slate-200 focus:border-primary-500 focus:ring-4 focus:ring-primary-500/10 rounded-xl px-4 py-2.5 text-slate-800 text-[13px] outline-none transition-all shadow-sm"
                    />
                  </div>
                  ...
                  <div>
                    ...
                    <input
                      ...
                      value={form.pcc}
                      onChange={(e) =>
                        setForm({ ...form, pcc: e.target.value })
                      }
                      className="w-full bg-white/70 border border-slate-200 focus:border-primary-500 focus:ring-4 focus:ring-primary-500/10 rounded-xl px-4 py-2.5 text-slate-800 text-[13px] outline-none transition-all shadow-sm"
                    />
                  </div>
                  <div className="md:col-span-2">
                    ...
                    <select
                      ...
                      onChange={(e) =>
                        setForm({ ...form, jobStatus: e.target.value })
                      }
                      className="w-full md:w-1/2 bg-white/70 border border-slate-200 focus:border-primary-500 focus:ring-4 focus:ring-primary-500/10 rounded-xl px-4 py-2.5 text-slate-800 text-[13px] outline-none transition-all shadow-sm"
                    >
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 gap-8 p-8 bg-white/50 rounded-2xl border border-white">
                  {infoRows.map((row) => (
                    ...
                    >
                      <span className="text-[10px] font-extrabold text-indigo-900/60 uppercase tracking-widest truncate">
                        {row.label}
                      </span>
                      {row.label === "Clients" ? (
                        <div className="flex flex-wrap gap-1.5">
                          {row.value !== "--" && row.value ? (
                            row.value.split(",").map((tag, i) => (
                              <span
                                key={i}
                                className="bg-white text-slate-700 border border-slate-200 px-2.5 py-1 rounded-md text-[12px] font-semibold shadow-sm"
                              >
                                {tag.trim()}
                              </span>
                            ))
                        ...
                        <span
                          className={`text-[14px] font-black break-words ${
            ...
            >
              <div className="p-2">
                <p className="text-slate-500 text-[13px] mb-6 max-w-3xl">
                  Define commission percentage slabs for{" "}
                  <strong className="text-slate-800">{agent.name}</strong>. The
                  system matches the agent's net profit to a slab below and
                  automatically applies the corresponding margin rate.
                </p>
                ...
                <div className="rounded-2xl overflow-hidden border border-slate-200 mb-6 shadow-sm bg-white/50">
                  ...
                  <div className="divide-y divide-slate-100/50">
                    {segments.map((seg, i) => (
                      <div
                        key={i}
                        className="grid grid-cols-12 items-center px-5 py-3 hover:bg-slate-50/50 transition-colors gap-3"
                      >
                        ...
                        <div className="col-span-3">
                          <input
                            ...
                            value={seg.minAmount}
                            onChange={(e) =>
                              updateSegment(i, "minAmount", e.target.value)
                            }
                        ...
                        <div className="col-span-3">
                          <input
                            ...
                            value={seg.maxAmount}
                            onChange={(e) =>
                              updateSegment(i, "maxAmount", e.target.value)
                            }
                            className="w-full bg-white/80 border border-slate-200 focus:border-primary-500 focus:ring-2 focus:ring-primary-500/10 rounded-xl px-3 py-1.5 text-slate-800 text-[13px] font-medium outline-none text-right transition-all"
                            placeholder="∞"
                          />
                        ...
                        <div className="col-span-1">
                          <input
                            ...
                            onChange={(e) =>
                              updateSegment(i, "marginPercent", e.target.value)
                            }
                        ...
                        <div className="col-span-1 flex justify-end">
                          <button
                            ...
                            className="p-1.5 rounded-xl hover:bg-rose-100 text-slate-400 hover:text-rose-500 transition-all"
                          >
                ...
                <div className="flex items-center gap-4 mt-6">
                  <button
                    onClick={addSegment}
                    className="flex items-center gap-1.5 px-4 py-2 rounded-xl bg-white hover:bg-slate-50 text-slate-600 text-[12px] font-bold border border-slate-200 shadow-sm transition-all active:scale-95"
                  >
              ...
              action={
                <div className="flex items-center gap-2">
                  <div className="flex flex-col items-end mr-4">
                    ...
                    <span
                      className={`text-base leading-none font-black ${agent.wallet && Number(agent.wallet.currentBalance) < 0 ? "text-rose-600" : "text-emerald-600"}`}
                    >
                      {agent.wallet
                        ? format(agent.wallet.currentBalance)
                        : format(0)}
                    </span>
                  </div>
                </div>
                      ...
                      <table className="w-full text-left text-[13px]">
                        ...
                        <tbody className="divide-y divide-slate-100/50 font-medium text-[12px]">
                          {agent.wallet.transactions
                            .slice(
                              (walletPage - 1) * walletItemsPerPage,
                              walletPage * walletItemsPerPage,
                            )
                            .map((tx) => (
                              <tr
                                key={tx.id}
                              ...
                              >
                                ...
                                <td
                                  className={`px-6 py-2.5 text-right font-bold ${Number(tx.amount) > 0 ? "text-emerald-600" : "text-rose-600"}`}
                                >
                                  {Number(tx.amount) > 0 ? "+" : ""}
                                  {format(tx.amount)}
                                </td>
                    ...
                    {agent.wallet.transactions.length > 0 && (
                      <Pagination
                        ...
                        totalPages={Math.ceil(
                          agent.wallet.transactions.length / walletItemsPerPage,
                        )}
    ...
    try {
      ...
      setError(
        err?.response?.data?.message ||
          `Failed to ${initialData ? "update" : "create"} agent`,
      );
    } finally {
      setSaving(false);
    }
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      >
        ...
        <div className="bg-slate-900 text-white px-4 py-3 flex items-center justify-between shrink-0">
          <h3 className="text-[11px] font-bold tracking-wide uppercase">
            {initialData ? "Edit Agent Profile" : "New Agent Profile"}
          </h3>
          <button
            type="button"
            onClick={onClose}
        ...
        <form
          ...
          className="flex flex-col flex-1 max-h-[85vh] overflow-hidden"
        >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="md:col-span-2">
                ...
                <input
                  ...
                  className="w-full bg-white border border-slate-200 focus:border-primary-500 focus:ring-4 focus:ring-primary-500/10 rounded-xl px-3 py-2 text-slate-800 text-[13px] font-bold outline-none transition-all placeholder:text-slate-400"
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full bg-white border border-slate-200 focus:border-primary-500 focus:ring-4 focus:ring-primary-500/10 rounded-xl px-3 py-2 text-slate-800 text-[13px] font-medium outline-none transition-all placeholder:text-slate-400"
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full bg-white border border-slate-200 focus:border-primary-500 focus:ring-4 focus:ring-primary-500/10 rounded-xl px-3 py-2 text-slate-800 text-[13px] font-medium outline-none transition-all placeholder:text-slate-400"
                  placeholder="sarah.payroll@example.com"
                />
              </div>
    ...
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 mb-6">
        <div>
          ...
          <p className="text-slate-500 text-xs mt-0.5">
            Manage agents, clients, and their commission margin slabs
          </p>
        </div>
        <div className="flex items-center gap-2">
          <button
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
        {[
          ...
          {
            label: "Active Agents",
            value: agents.filter((a) => a.jobStatus === "Active").length,
          ...
          <div
            key={stat.label}
            className={`bg-white border border-slate-200 rounded-3xl p-6 flex flex-col justify-center shadow-sm relative overflow-hidden group`}
          >
            <div
              className={`absolute right-0 top-0 w-24 h-24 ${stat.bg} rounded-bl-full -z-10 opacity-70 group-hover:scale-110 transition-transform duration-500`}
            />
            ...
            <div className="flex items-end gap-3">
              ...
              <div
                className={`mb-1.5 w-8 h-8 rounded-lg ${stat.bg} ${stat.color} flex items-center justify-center`}
              >
      ...
      {loading ? (
        ...
        <EmptyState
          ...
          action={
            <button
              onClick={() => setShowAddModal(true)}
              className="flex items-center gap-1.5 bg-primary-600 text-white hover:bg-primary-500 px-5 py-2.5 rounded-xl text-xs font-bold shadow-md shadow-primary-500/20 active:scale-95 transition-all"
            >
                  ...
                  <table className="w-full text-left border-collapse">
                    ...
                    <tbody className="divide-y divide-slate-50 text-slate-600 font-medium">
                      {tableAgents.map((agent) => (
                        ...
                        >
                          <td className="py-3.5 px-5 font-black text-slate-500">
                            #{agent.id.toString().padStart(4, "0")}
                          </td>
                              ...
                              {agent.client ? (
                                agent.client
                                  ...
                                  .map((tag, i) => (
                                    <span
                                      key={i}
                                      className="bg-slate-50 text-slate-600 border border-slate-100 px-1.5 py-0.5 rounded text-[9px] font-bold"
                                    >
                                      {tag.trim()}
                                    </span>
                                  ))
                          ...
                          >
                            <div className="flex items-center justify-center gap-2">
                              ...
                              <button
                                onClick={() => setSelectedAgent(agent)}
                                className="bg-primary-50 text-primary-600 hover:bg-primary-100 font-bold px-3 py-1.5 rounded-lg transition-colors ml-2"
                              >
                                Inspect
                              </button>
                            </div>
            ...
            onSaved={() => {
              ...
              if (selectedAgent) {
                const updated = agents.find(
                  (a) => a.id === (selectedAgent as any)?.id,
                );
