  ...
  const fetchVendorHistory = async (vendorName: string) => {
    try {
      ...
      const walletTxns = rawTxns.filter((t: any) => {
        ...
        return desc.includes('overpayment') || 
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
        ...
        expensesLoading ? (
          ...
          <div className="bg-white border border-slate-200 rounded-3xl shadow-sm overflow-hidden text-[13px]">
            <div className="px-6 py-4 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
              <h3 className="font-bold text-slate-900">Registered Corporate Expenses</h3>
              <button
                onClick={() => {
                  setEditingExpense(null);
                  setExpenseName('');
                      ...
                      <tr key={exp.id} className="hover:bg-slate-50/70 transition-colors">
                        ...
                        <td className="py-4 px-6">
                          <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-[9px] font-extrabold uppercase tracking-wide border ${
                            ...
                            {exp.type === 'recurring' ? 'Recurring Monthly' : 'One-Time'}
                          </span>
                        </td>
                        <td className="py-4 px-6 text-slate-500 font-semibold">{new Date(exp.date).toLocaleDateString()}</td>
                        <td className="py-4 px-6 text-slate-500 italic max-w-xs truncate">{exp.notes || 'No description'}</td>
                        <td className="py-4 px-6 text-right">
      ...
      <AnimatePresence>
        ...
        {showAddExpenseModal && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 animate-in fade-in duration-200">
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={() => setShowAddExpenseModal(false)} className="absolute inset-0 bg-slate-900/60 backdrop-blur-sm" />
            <motion.div initial={{ opacity: 0, scale: 0.95, y: 15 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95 }}
            ...
            >
              <div className="bg-gradient-to-r from-rose-600 to-rose-700 text-white px-6 py-4 flex justify-between items-center shadow-inner shrink-0">
                <h3 className="font-extrabold text-white text-sm tracking-wide uppercase leading-none">{editingExpense ? 'Modify Corporate Expense' : 'Record Corporate Expense'}</h3>
                <button onClick={() => setShowAddExpenseModal(false)} className="text-white/60 hover:text-white transition-colors"><X className="w-5 h-5" /></button>
              </div>

              <form onSubmit={handleSaveExpense} className="p-6 space-y-4 text-xs font-semibold text-slate-600">
                <div>
                  <label className="block text-[11px] font-bold text-slate-500 uppercase tracking-wider mb-2">Expense Name / Title *</label>
                  <input
                    type="text"
                    required
                    value={expenseName}
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    <label className="block text-[11px] font-bold text-slate-500 uppercase tracking-wider mb-2">Billing Frequency *</label>
                    <select
                      value={expenseType}
                ...
                <div>
                  <label className="block text-[11px] font-bold text-slate-500 uppercase tracking-wider mb-2">Billing Start / Occur Date *</label>
                  <input
                    type="date"
                    required
                    value={expenseDate}
                ...
                <div className="bg-slate-50 px-6 py-4 border-t border-slate-200 flex justify-end gap-3 -mx-6 -mb-6 mt-6 shrink-0 font-bold">
                  <button type="button" onClick={() => setShowAddExpenseModal(false)} className="px-5 py-2.5 rounded-xl text-[13px] text-slate-600 hover:bg-slate-200/50 transition-all">Cancel</button>
                  <button type="submit" disabled={expenseSubmitting} className="px-6 py-2.5 rounded-xl bg-rose-600 hover:bg-rose-500 text-white text-[13px] shadow-md shadow-rose-600/25 transition-all flex items-center gap-2 active:scale-95">
                    {expenseSubmitting ? <Loader2 className="w-4 h-4 animate-spin" /> : (editingExpense ? <Check className="w-4 h-4" /> : <Plus className="w-4 h-4" />)}
        ...
        {deletingExpense && (
          <div className="fixed inset-0 z-[90] flex items-center justify-center p-4">
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="absolute inset-0 bg-slate-900/60 backdrop-blur-md" onClick={() => setDeletingExpense(null)} />
            <motion.div initial={{ opacity: 0, scale: 0.95, y: 10 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95, y: 10 }}
              className="bg-white/80 backdrop-blur-xl border border-white/50 rounded-2xl shadow-2xl w-full max-w-sm relative z-10 overflow-hidden flex flex-col p-6 text-center">
              <div className="w-16 h-16 rounded-full bg-rose-100/50 backdrop-blur-sm flex items-center justify-center mx-auto mb-4 text-rose-600">
                <Trash2 className="w-8 h-8" />
              ...
              <p className="text-slate-500 text-sm mb-6">Are you sure you want to delete the expense "{deletingExpense.name}"? This will remove it from the ledger.</p>
              <div className="flex gap-3">
                <button onClick={() => setDeletingExpense(null)} className="flex-1 px-4 py-2.5 rounded-xl text-[13px] font-bold text-slate-600 hover:bg-slate-100 transition-all border border-slate-200">Cancel</button>
                ...
                }} disabled={actionLoading} className="flex-1 px-4 py-2.5 rounded-xl bg-rose-600 hover:bg-rose-500 text-white text-[13px] font-bold shadow-md transition-all flex items-center justify-center gap-2">
                  {actionLoading ? <div className="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin" /> : 'Delete'}
                </button>
              </div>
            </motion.div>
