...
function GeneratePayrollModal({
  ...
  const handleGenerate = async () => {
    ...
    setLoading(true);
    try {
      await api.post(`/agents/${agentId}/payroll`, {
        periodFrom,
        periodTo,
      ...
      >
          ...
          <div className="relative z-10 flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-[11px] text-indigo-200">
                Calculate salary + commission for a period
              </p>
            </div>
          </div>
          <button
        ...
        <div className="p-6 space-y-4">
            ...
            >
              ...
              {agents.map((a) => (
                <option key={a.id} value={a.id}>
                  {a.name}{" "}
                  {a.basicSalary
                    ? `(${symbol}${Number(a.basicSalary).toFixed(2)}/mo)`
                    : "(no salary set)"}
                </option>
              ))}
            </select>
          ...
          <div className="bg-blue-50 border border-blue-100 rounded-xl p-3 text-[12px] text-blue-700">
            <strong>💡 How it works:</strong> We'll calculate total commission
            earned from all non-cancelled bookings in the selected period where
            this agent is assigned. Basic salary is taken from the agent's
            profile.
          </div>
        ...
        <div className="p-6 space-y-4">
          <div>
            ...
            <p className="mt-1.5 text-[10px] text-slate-400">
              Leave blank to remove the salary. This is used in payroll
              calculations.
            </p>
          </div>
...
export function PayrollPage() {
  ...
  const totalMargin = payrolls.reduce(
    (s, p) => s + Number(p.totalMarginEarned),
    0,
  );
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
        <div>
          ...
          <p className="text-slate-500 text-xs mt-1">
            Manage salaries, commissions, and send payroll slips.
          </p>
        </div>
        <button
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {[
          ...
          },
          {
            ...
            icon: Wallet,
            color: "amber",
            sub: "Basic salary sum",
          },
          {
            label: "Total Commission",
            value: format(totalMargin),
            icon: TrendingUp,
            color: "emerald",
            sub: "Margin earned",
          },
          ...
          >
            <div
              className={`w-9 h-9 rounded-xl bg-${card.color}-50 flex items-center justify-center mb-3`}
            >
      ...
      <div className="bg-white border border-slate-100 rounded-2xl shadow-sm overflow-hidden">
        <div className="px-5 py-4 border-b border-slate-100 flex items-center justify-between">
          ...
          <p className="text-[11px] text-slate-400">
            Click pencil to set basic salary per agent
          </p>
        </div>
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="py-3 px-5 text-slate-600 font-medium whitespace-nowrap">
                        {new Date(p.periodFrom).toLocaleDateString("en-GB", {
                          day: "numeric",
                          month: "short",
                        })}{" "}
                        –{" "}
                        {new Date(p.periodTo).toLocaleDateString("en-GB", {
                          day: "numeric",
                          month: "short",
                          year: "numeric",
                        })}
                      </td>
                      <td className="py-3 px-5 text-right font-bold text-slate-700">
                        {format(p.basicSalary)}
                      </td>
                      <td className="py-3 px-5 text-right font-bold text-emerald-600">
                        +{format(p.totalMarginEarned)}
                      </td>
                      <td className="py-3 px-5 text-right">
                        <span className="text-[14px] font-extrabold text-primary-700">
                          {format(p.totalPaid)}
                        </span>
                      </td>
                      <td className="py-3 px-5">
                        <span
                          className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg border text-[11px] font-bold ${STATUS_STYLES[p.status]}`}
                        >
                        ...
                        {p.sentAt && (
                          <p className="text-[10px] text-slate-400 mt-0.5">
                            Sent{" "}
                            {new Date(p.sentAt).toLocaleDateString("en-GB")}
                          </p>
                        )}
                      </td>
                      <td className="py-3 px-5 text-slate-500 max-w-[120px] truncate text-[11px]">
                        {p.notes || "--"}
                      </td>
                      <td className="py-3 px-5">
...
function SendPayrollSlipModal({
  payroll,
  onClose,
  onSent,
}: SendPayrollSlipModalProps) {
  const { symbol } = useCurrency();
  const [email, setEmail] = useState(
    payroll.agent.personalEmail || payroll.agent.email || "",
  );
  const [loading, setLoading] = useState(false);
  ...
  const [totalWorkdays, setTotalWorkdays] = useState(() =>
    payroll.totalWorkdays !== null && payroll.totalWorkdays !== undefined
      ? Number(payroll.totalWorkdays)
      : 22,
  );
  const [daysPresent, setDaysPresent] = useState(() =>
    payroll.daysPresent !== null && payroll.daysPresent !== undefined
      ? Number(payroll.daysPresent)
      : 22,
  );
  const [absents, setAbsents] = useState(() =>
    payroll.absents !== null && payroll.absents !== undefined
      ? Number(payroll.absents)
      : 0,
  );

  const [paidHolidaysCount, setPaidHolidaysCount] = useState(() =>
    payroll.paidHolidaysCount !== null &&
    payroll.paidHolidaysCount !== undefined
      ? Number(payroll.paidHolidaysCount)
      : 0,
  );
  const [paidHolidaysRate, setPaidHolidaysRate] = useState(() =>
    payroll.paidHolidaysRate !== null && payroll.paidHolidaysRate !== undefined
      ? Number(payroll.paidHolidaysRate)
      : 0,
  );
  const [publicHolidaysCount, setPublicHolidaysCount] = useState(() =>
    payroll.publicHolidaysCount !== null &&
    payroll.publicHolidaysCount !== undefined
      ? Number(payroll.publicHolidaysCount)
      : 0,
  );
  const [publicHolidaysRate, setPublicHolidaysRate] = useState(() =>
    payroll.publicHolidaysRate !== null &&
    payroll.publicHolidaysRate !== undefined
      ? Number(payroll.publicHolidaysRate)
      : 0,
  );
  ...
  >(() => {
    if (payroll.allowances !== null && payroll.allowances !== undefined) {
      return typeof payroll.allowances === "string"
  ...
  >(() => {
    if (payroll.deductions !== null && payroll.deductions !== undefined) {
      return typeof payroll.deductions === "string"
  ...
  useEffect(() => {
    ...
    api
      .get(`/agents/${payroll.agentId}/attendance`, {
        params: {
          from: payroll.periodFrom.slice(0, 10),
          to: payroll.periodTo.slice(0, 10),
        },
      })
      .then((attRes) => {
        if (!active) return;
        ...
        if (!isSaved) {
          ...
          const computedAbsents = Math.max(
            0,
            defaultTotalWorkdays - computedPresent,
          );
          setAbsents(computedAbsents);

          const computedDailyRate =
            defaultTotalWorkdays > 0
              ? basicSalaryVal / defaultTotalWorkdays
              : 0;
          ...
          const initialAllowances = [
            {
              description:
                "Travel & Internet connectivity allowance (Amenities)",
              amount: travelAllowance,
            },
          ];
          if (gdsAllowance > 0) {
            initialAllowances.push({
  ...
  const handleWorkdaysChange = (val: number) => {
    ...
    const computedDailyRate =
      sanitizedVal > 0 ? basicSalaryVal / sanitizedVal : 0;
    setPaidHolidaysRate(Number(computedDailyRate.toFixed(2)));
    setPublicHolidaysRate(Number((computedDailyRate * 1.5).toFixed(2)));
  };
  ...
  const addAllowance = () => {
    ...
    setAllowances([
      ...allowances,
      { description: newAllowDesc, amount: Math.abs(Number(newAllowAmt)) },
    ]);
    setNewAllowDesc("");
    setNewAllowAmt("");
  };
  ...
  const addDeduction = () => {
    ...
    setDeductions([
      ...deductions,
      { description: newDedDesc, amount: Math.abs(Number(newDedAmt)) },
    ]);
    setNewDedDesc("");
    setNewDedAmt("");
  };
  ...
  const periodToStr = new Date(payroll.periodTo).toLocaleDateString("en-GB", {
    day: "2-digit",
    month: "short",
    year: "numeric",
  });
          ...
          <div className="relative z-10 flex items-center gap-3">
            ...
            <div>
              <h3 className="text-[16px] font-extrabold">
                Customize & Send Salary Slip
              </h3>
              <p className="text-[11px] text-indigo-100">
                Tailor wages, calendar parameters, and attachments before
                dispatch
              </p>
            </div>
          </div>
        ...
        {loadingData ? (
          ...
          <div className="flex-1 overflow-hidden flex flex-col lg:flex-row print:block">
              ...
              <div className="bg-slate-50 rounded-2xl p-4 border border-slate-100 space-y-3">
                ...
                <div>
                  ...
                  <input
                    ...
                    value={totalWorkdays}
                    onChange={(e) =>
                      handleWorkdaysChange(Number(e.target.value))
                    }
                    className="w-full border border-slate-200 rounded-xl px-3 py-2 text-slate-800 text-[12px] outline-none focus:border-indigo-500"
                  />
                </div>
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    ...
                    <input
                      ...
                      value={daysPresent}
                      onChange={(e) =>
                        handlePresentChange(Number(e.target.value))
                      }
                      className="w-full border border-slate-200 rounded-xl px-3 py-2 text-slate-800 text-[12px] outline-none focus:border-indigo-500"
                    />
                  </div>
                  <div>
                    ...
                    <input
                      ...
                      value={absents}
                      onChange={(e) =>
                        setAbsents(Math.max(0, Number(e.target.value)))
                      }
                      className="w-full border border-slate-200 rounded-xl px-3 py-2 text-slate-800 text-[12px] outline-none focus:border-indigo-500"
                    />
                  </div>
                ...
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setPaidHolidaysRate(Math.max(0, Number(e.target.value)))
                      }
                      className="w-full border border-slate-200 rounded-xl px-3 py-2 text-slate-800 text-[12px] outline-none focus:border-indigo-500"
                    />
                  </div>
                ...
                {allowances.length > 0 && (
                  <div className="space-y-1.5 max-h-36 overflow-y-auto pr-1">
                    {allowances.map((item, index) => (
                      <div
                        key={index}
                        className="flex justify-between items-center bg-white border border-slate-200 rounded-lg p-2 text-xs"
                      >
            ...
            <div className="flex-1 bg-slate-50 overflow-y-auto p-6 flex flex-col justify-between print:bg-white print:p-0">
              <div
                id="printable-custom-slip"
                className="bg-white border border-slate-200 rounded-2xl p-6 shadow-sm text-slate-800 print:border-none print:shadow-none print:rounded-none"
              >
                <div className="flex justify-between items-start border-b border-slate-200 pb-5 mb-5">
                  ...
                  <div className="text-right">
                    ...
                    <p className="text-[9px] text-slate-400 mt-2 font-medium">
                      Issue Date:{" "}
                      {new Date(payroll.createdAt).toLocaleDateString("en-GB")}
                    </p>
                  </div>
                </div>
                ...
                <div className="bg-slate-50 border border-slate-100 rounded-xl p-3.5 mb-5 print:bg-slate-50">
                  <h5 className="text-[9px] uppercase font-black text-slate-400 tracking-wider mb-2">
                    Attendance Summary
                  </h5>
                  <div className="grid grid-cols-3 gap-3 text-center">
                    ...
                    <div>
                      <span className="block text-[14px] font-black text-emerald-600">
                        {daysPresent}
                      </span>
                      <span className="block text-[8px] uppercase font-bold text-slate-400 tracking-wider">
                        Present
                    ...
                    <div>
                      <span className="block text-[14px] font-black text-red-650">
                        {absents}
                      </span>
                      <span className="block text-[8px] uppercase font-bold text-slate-400 tracking-wider">
                        Absent
                ...
                <table className="w-full text-left border-collapse text-[11px] mb-5">
                  ...
                  <tbody className="divide-y divide-slate-100">
                    ...
                    {paidHolidaysCount > 0 && (
                      <tr>
                        <td className="py-2 px-3 font-semibold text-slate-800">
                          Paid Holidays ({paidHolidaysCount} days @ {symbol}
                          {paidHolidaysRate.toFixed(2)}/day)
                        </td>
                        <td className="py-2 px-3 text-right font-bold text-slate-800">
                          +{symbol}
                          {(paidHolidaysCount * paidHolidaysRate).toFixed(2)}
                        </td>
                        <td className="py-2 px-3 text-right text-slate-350">
                          &mdash;
                        </td>
                    ...
                    {publicHolidaysCount > 0 && (
                      <tr>
                        <td className="py-2 px-3 font-semibold text-slate-800">
                          Public Holidays ({publicHolidaysCount} days @ {symbol}
                          {publicHolidaysRate.toFixed(2)}/day)
                        </td>
                        <td className="py-2 px-3 text-right font-bold text-slate-800">
                          +{symbol}
                          {(publicHolidaysCount * publicHolidaysRate).toFixed(
                            2,
                          )}
                        </td>
                        <td className="py-2 px-3 text-right text-slate-350">
                    ...
                    {absents > 0 && dailyRate > 0 && (
                      <tr>
                        <td className="py-2 px-3 font-semibold text-slate-800">
                          Absenteeism Penalty ({absents} days absent @ {symbol}
                    ...
                    <tr className="bg-slate-50 font-bold border-t border-slate-200">
                      ...
                      <td className="py-2 px-3 text-right text-slate-850">
                        {symbol}
                        {grossEarnings.toFixed(2)}
                      </td>
                      <td className="py-2 px-3 text-right text-red-650">
                        {symbol}
                    ...
                    <tr className="bg-gradient-to-r from-slate-900 to-indigo-950 text-white font-bold">
                      <td className="py-3 px-4 rounded-l-xl text-[12px] border-none">
                        Total Net Payable (Net Salary)
                      </td>
                      <td
                        colSpan={2}
                        className="py-3 px-4 text-right text-[14px] text-sky-400 font-black rounded-r-xl border-none"
                      >
                        {symbol}
                ...
                {checkingSmtp ? (
                  ...
                  <div className="bg-amber-50 border border-amber-100 rounded-xl p-2.5 text-[10.5px] text-amber-800 flex items-start gap-2">
                    ...
                    <div>
                      ...
                      Configure SMTP in Dashboard settings to send branded
...
function ViewSalarySlipModal({ payroll, onClose }: ViewSalarySlipModalProps) {
  ...
  const totalWorkdays = isSaved
    ? Number(payroll.totalWorkdays)
    : defaultTotalWorkdays;
  const daysPresent = isSaved ? Number(payroll.daysPresent) : uniqueDays.size;
  const absents = isSaved
    ? Number(payroll.absents)
    : Math.max(0, totalWorkdays - daysPresent);

  const paidHolidaysCount = isSaved
    ? Number(payroll.paidHolidaysCount || 0)
    : 0;
  const paidHolidaysRate = isSaved ? Number(payroll.paidHolidaysRate || 0) : 0;
  const publicHolidaysCount = isSaved
    ? Number(payroll.publicHolidaysCount || 0)
    : 0;
  const publicHolidaysRate = isSaved
    ? Number(payroll.publicHolidaysRate || 0)
    : 0;
  ...
  const defaultGdsAllowance = payroll.agent?.gdsSystem ? 120.0 : 0.0;
  const defaultTravelAllowance = 80.0;
  const defaultAllowances = [
    {
      description: "Travel & Internet connectivity allowance (Amenities)",
      amount: defaultTravelAllowance,
    },
  ];
  if (defaultGdsAllowance > 0) {
    defaultAllowances.push({
      description: "GDS Terminal Premium allowance (Amenities)",
      amount: defaultGdsAllowance,
    });
  }

  const allowances: { description: string; amount: number }[] =
    isSaved && payroll.allowances
  ...
  const customDeductions: { description: string; amount: number }[] =
    isSaved && payroll.deductions
      ? typeof payroll.deductions === "string"
        ? JSON.parse(payroll.deductions)
        : payroll.deductions
      : [];

  const customDeductionsVal = customDeductions.reduce(
    ...
    >
      ...
      >
        ...
        <div className="overflow-y-auto p-6 flex-1 print:overflow-visible print:p-0">
          {loading ? (
            <div className="py-12">
              <LoadingState message="Compiling salary slip details..." />
            </div>
          ) : (
            <div
              id="printable-slip"
              className="bg-white border border-slate-200 rounded-2xl p-6 shadow-sm print:border-none print:shadow-none print:rounded-none text-slate-800"
            >
              <div className="flex justify-between items-start border-b border-slate-200 pb-5 mb-5">
                <div>
                  {logoUrl && (
              ...
              <div className="bg-slate-50 border border-slate-100 rounded-xl p-4 mb-6">
                ...
                <div className="grid grid-cols-3 gap-4 text-center">
                  <div>
                    ...
                    <span className="block text-[9px] uppercase font-bold text-slate-400 tracking-wider mt-0.5">
                      Total Workdays
                    </span>
                  </div>
                  <div>
                  ...
                  <div>
                    <span className="block text-[15px] font-extrabold text-red-600">
                      {absents}
                    </span>
                    <span className="block text-[9px] uppercase font-bold text-slate-400 tracking-wider mt-0.5">
                      Days Absent
                ...
                <tbody className="divide-y divide-slate-100">
                  ...
                  {publicHolidaysCount > 0 && (
                    <tr>
                      ...
                      <td className="py-3 px-3 text-right font-bold text-slate-800">
                        +{symbol}
                        {(publicHolidaysCount * publicHolidaysRate).toFixed(2)}
                      </td>
                      <td className="py-3 px-3 text-right text-slate-300">
                        &mdash;
                      </td>
                  ...
                  {absentDeduction > 0 && (
                    <tr>
                      <td className="py-3 px-3 font-semibold text-slate-800">
                        Absenteeism Penalty ({absents} day
                        {absents !== 1 ? "s" : ""} @ {symbol}
                        {dailyRate.toFixed(2)}/day)
                      </td>
                      <td className="py-3 px-3 text-right text-slate-300">
                  ...
                  <tr className="bg-gradient-to-r from-slate-900 to-indigo-950 text-white font-bold">
                    <td className="py-3.5 px-4 rounded-l-xl text-[13px] border-none">
                      Total Net Payable (Net Salary)
                    </td>
                    <td
                      colSpan={2}
                      className="py-3.5 px-4 text-right text-[15px] text-sky-400 font-black rounded-r-xl border-none"
                    >
                      {symbol}
                      {finalNetPay.toFixed(2)}
                    </td>
