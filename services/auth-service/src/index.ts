...
function getRequiredPermissionForPath(
  path: string,
  method: string,
): string | null {
  ...
  if (cleanPath.startsWith("/agents/payroll/")) {
    ...
    if (segments.length === 4 && segments[3] === "send") {
      // /agents/payroll/:id/send
  ...
  if (cleanPath.startsWith("/agents/")) {
    const segments = cleanPath.split("/");
    if (segments.length === 4 && segments[3] === "attendance") {
      // /agents/:id/attendance
      return "READ_ATTENDANCE";
    }
    if (
      segments.length === 5 &&
      segments[3] === "attendance" &&
      segments[4] === "checkin"
    ) {
      // /agents/:id/attendance/checkin
    ...
    if (
      ...
      // /agents/:id/attendance/checkout
      return "CREATE_ATTENDANCE";
    }
    if (segments.length === 4 && segments[3] === "payroll") {
      // /agents/:id/payroll
      return "CREATE_PAYROLL";
    }
  ...
  if (userRole === "AGENT") {
    ...
    if (isNaN(uId) || isNaN(tId)) {
      return res
        ...
        .json({
          error: "Forbidden",
          message: "Permission Denied: Invalid user or tenant ID",
        });
    }
    ...
    try {
      ...
      if (user.tenantId !== tId) {
        return res
          ...
          .json({
            error: "Forbidden",
            message: "Permission Denied: Tenant context mismatch",
          });
      }
      ...
      if (user.role?.name !== "AGENT") {
        return res
          ...
          .json({
            error: "Forbidden",
            message: "Permission Denied: Role mismatch",
          });
      }
      ...
      if (requiredPermission) {
        ...
        if (!hasPermission) {
          return res
            ...
            .json({
              error: "Forbidden",
              message: `Permission Denied: Missing ${requiredPermission} permission`,
            });
        }
      }
    } catch (error) {
...
app.post("/upload", upload.single("file"), async (req: any, res: Response) => {
  try {
    ...
    if (!externalUrl) {
      ...
      const isProd = [host, origin, referer, forwardedHost].some(
        (h) =>
          typeof h === "string" && h.toLowerCase().includes("techbarred.com"),
      );

      externalUrl = isProd
        ? "https://bucket.techbarred.com"
        : "http://localhost:9010";
    }
  ...
  try {
    ...
    if (existingUser) {
      return res
        ...
        .json({
          error:
            "This email is already registered in the system. Users cannot cross-login across different companies.",
        });
    }
  ...
  try {
    ...
    if (parsedData.isSuperAdmin) {
      ...
      if (!admin) {
        return res
          .status(401)
          .json({ error: "Invalid platform admin credentials" });
      }
    ...
    if (tenant.status === "suspended") {
      return res
        .status(403)
        .json({ error: "Company account is suspended. Please contact admin." });
    }
    ...
    if (tenant.subscriptionPlan === "trial") {
      if (tenant.trialEndsAt && new Date() > new Date(tenant.trialEndsAt)) {
        return res.status(402).json({
          ...
          message:
            "Your company's free trial has ended. Please ask your administrator to purchase a lifetime or active subscription plan.",
        });
      }
      ...
      return res.status(402).json({
        ...
        message:
          "Your company's subscription has expired. Please contact support.",
      });
    }
...
app.post("/forgot-password", async (req: any, res: Response) => {
  try {
    ...
    console.log(
      `Password reset link sent to ${user.email} using Super Admin SMTP`,
    );
  ...
  try {
    ...
    if (parsedData.domain) {
      ...
      if (existingDomain) {
        return res
          .status(409)
          .json({ error: "Tenant domain already registered" });
      }
    }
    ...
    if (existingEmail) {
      return res
        ...
        .json({
          error:
            "This admin email is already registered to another company. Users cannot cross-login across different companies.",
        });
    }
    ...
    const newTenant = await prisma.$transaction(async (tx: any) => {
      ...
      const tenant = await tx.tenant.create({
        data: {
          ...
          subscriptionStatus:
            parsedData.subscriptionPlan === "trial" ? "trial" : "active",
          trialEndsAt,
...
const requireTenantContext = (req: any, res: Response, next: any) => {
  ...
  if (!tenantId || tenantId === "platform") {
    return res
      .status(400)
      .json({ error: "Bad Request", message: "Tenant context required" });
  }
    ...
    try {
      ...
      const updatedTenant = await (prisma as any).tenant.update({
        ...
        data: {
          ...
          ...(smtpPort !== undefined && {
            smtpPort:
              smtpPort === null || smtpPort === "" ? null : parseInt(smtpPort),
          }),
  ...
  async (req: any, res: Response) => {
    try {
      ...
      if (parsedData.trialEndsAt !== undefined) {
        updateData.trialEndsAt = parsedData.trialEndsAt
          ? new Date(parsedData.trialEndsAt)
          : null;
      }
  ...
  async (req: any, res: Response) => {
    try {
      ...
      if (
        ...
        console.log(
          `Using custom SMTP for tenant ${tenantId}: ${tenant.smtpHost}`,
        );
...
app.get("/agents/verify-request", async (req: any, res: Response) => {
  ...
  if (!userId || !tenantIdStr) {
    return res
      ...
      .json({
        allowed: false,
        message: "Missing user or tenant context headers",
      });
  }
  ...
  if (isNaN(uId) || isNaN(tId)) {
    return res
      .status(200)
      .json({ allowed: false, message: "Invalid user or tenant ID" });
  }
  ...
  try {
    ...
    if (!user) {
      return res
        .status(200)
        .json({ allowed: false, message: "User not found" });
    }

    if (user.tenantId !== tId) {
      return res
        .status(200)
        .json({ allowed: false, message: "Tenant context mismatch" });
    }
  ...
  async (req: any, res: Response) => {
    try {
      ...
      if (!agent)
        return res
          .status(404)
          .json({ error: "Not Found", message: "Agent not found" });
...
app.get("/agents/attendance", async (req: Request, res: Response) => {
  try {
    ...
    const todayRecords = await prisma.agentAttendance.findMany({
      where: { tenantId, checkIn: { gte: todayStart, lte: todayEnd } },
    });
    const currentlyIn = await prisma.agentAttendance.count({
...
app.post(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      return res
        .status(201)
        .json({ message: "Checked in successfully", record });
    } catch (err) {
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const durationMinutes = Math.round(
        (checkOutTime.getTime() - new Date(openRecord.checkIn).getTime()) /
          60000,
      );
  ...
  async (req: any, res: Response) => {
    try {
      ...
      if (!periodFrom || !periodTo) {
        return res
          .status(400)
          .json({ error: "Missing required fields: periodFrom and periodTo" });
      }
      ...
      const basicSalary = agent.basicSalary ? Number(agent.basicSalary) : 0.0;

      // Call booking-service to get total margin earned
      const bookingServiceUrl =
        process.env.BOOKING_SERVICE_URL || "http://booking-service:4005";
      let totalMarginEarned = 0.0;

      try {
        const response = await axios.get(
          `${bookingServiceUrl}/finance/agent-margin-summary`,
          {
            ...
            headers: {
              ...
              "X-User-Role": String(
                req.headers["x-user-role"] || "COMPANY_ADMIN",
              ),
            },
          },
        );
        totalMarginEarned = response.data.totalMarginEarned
          ? parseFloat(response.data.totalMarginEarned)
          : 0.0;
      } catch (apiErr: any) {
        console.error(
          "Failed to fetch agent margin summary from booking-service:",
          apiErr.message,
        );
        return res
          .status(502)
          .json({
            error: "Failed to fetch agent margin summary from booking service",
          });
      }
      ...
      return res
        .status(201)
        .json({ message: "Payroll generated successfully", payroll });
    } catch (err: any) {
...
app.post(
  ...
  async (req: any, res: Response) => {
    try {
      ...
      const targetEmail =
        req.body.email || payroll.agent.personalEmail || payroll.agent.email;
      if (!targetEmail) {
        return res
          .status(400)
      ...
      const customTotalWorkdays =
        req.body.totalWorkdays !== undefined
          ? Number(req.body.totalWorkdays)
          : null;
      const customDaysPresent =
        req.body.daysPresent !== undefined
          ? Number(req.body.daysPresent)
          : null;
      const customAbsents =
        req.body.absents !== undefined ? Number(req.body.absents) : null;
      const paidHolidaysCount =
        req.body.paidHolidaysCount !== undefined
          ? Number(req.body.paidHolidaysCount)
          : 0;
      const paidHolidaysRate =
        req.body.paidHolidaysRate !== undefined
          ? Number(req.body.paidHolidaysRate)
          : 0;
      const publicHolidaysCount =
        req.body.publicHolidaysCount !== undefined
          ? Number(req.body.publicHolidaysCount)
          : 0;
      const publicHolidaysRate =
        req.body.publicHolidaysRate !== undefined
          ? Number(req.body.publicHolidaysRate)
          : 0;
      const allowances = req.body.allowances || [];
      const deductions = req.body.deductions || [];
      const customNotes =
        req.body.notes !== undefined ? req.body.notes : payroll.notes;
        ...
        const attendanceRecords = await prisma.agentAttendance.findMany({
          where: {
            ...
            checkIn: {
              ...
              lte: new Date(
                new Date(payroll.periodTo).setHours(23, 59, 59, 999),
              ),
            },
          },
        });
      ...
      const totalWorkdays =
        customTotalWorkdays !== null ? customTotalWorkdays : weekdaysCount || 1;
      const absents =
        customAbsents !== null
          ? customAbsents
          : Math.max(0, totalWorkdays - daysPresent);
      ...
      if (req.body.allowances === undefined) {
        const gdsAllowance = payroll.agent.gdsSystem ? 120.0 : 0.0;
        const travelAllowance = 80.0;
        finalAllowances = [
          {
            description: "Travel & Internet connectivity allowance (Amenities)",
            amount: travelAllowance,
          },
        ];
        if (gdsAllowance > 0) {
          finalAllowances.push({
            description: "GDS Terminal Premium allowance (Amenities)",
            amount: gdsAllowance,
          });
        }
      }
      ...
      const holidayPay =
        paidHolidaysCount * paidHolidaysRate +
        publicHolidaysCount * publicHolidaysRate;

      const grossEarnings =
        basicSalaryVal + marginEarnedVal + totalAllowances + holidayPay;

      const customDeductionsVal = deductions.reduce(
      ...
      const periodFromStr = new Date(payroll.periodFrom).toLocaleDateString(
        "en-GB",
        { day: "2-digit", month: "short", year: "numeric" },
      );
      const periodToStr = new Date(payroll.periodTo).toLocaleDateString(
        "en-GB",
        { day: "2-digit", month: "short", year: "numeric" },
      );
      const issueDateStr = new Date(payroll.createdAt).toLocaleDateString(
        "en-GB",
        { day: "2-digit", month: "short", year: "numeric" },
      );
    ...
    `;
      ...
      const { transporter, fromEmail, fromName } =
        await getTenantSmtpTransporter(tenantId);
      ...
      try {
        const bookingServiceUrl =
          process.env.BOOKING_SERVICE_URL || "http://booking-service:4005";
        await axios.post(
          `${bookingServiceUrl}/finance/payroll-ledger-entry`,
          {
            agentName: payroll.agent.name,
        ...
        console.error(
          "Failed to register payroll in ledger:",
          ledgerErr.response?.data || ledgerErr.message,
        );
      ...
      return res.json({
        message: "Payroll slip sent successfully and logged in ledger",
        payroll: updated,
      });
    } catch (err: any) {
      ...
      return res
        .status(500)
        .json({ error: "Failed to send payroll slip", message: err.message });
    }
  },
);
  ...
  async (req: any, res: Response) => {
    try {
      ...
      if (agentId !== undefined)
        dataToUpdate.agentId = agentId === null ? null : parseInt(agentId);
      ...
      if (resolvedRoleName === "AGENT") {
        try {
          let currentAgentId =
            dataToUpdate.agentId !== undefined
              ? dataToUpdate.agentId
              : user.agentId;
          if (!currentAgentId) {
        ...
        if (agentToDeleteId) {
          ...
          try {
            await (prisma as any).agent.delete({
              where: { id: agentToDeleteId },
            });
          } catch (e) {
            console.error(
              "Failed to delete agent profile on user role change:",
              e,
            );
          }
        }
    ...
    try {
      ...
      if (user.agentId) {
        try {
          ...
          console.error(
            "Failed to delete associated agent on user deletion:",
            e,
          );
        }
      }
...
app.post("/agents", requireTenantContext, async (req: any, res: Response) => {
  try {
    ...
    if (!name)
      return res
        .status(400)
        .json({ error: "Validation failed", message: "name is required" });
    ...
    try {
      ...
      const agent = await (prisma as any).agent.update({
        ...
        data: {
          ...
          personalEmail:
            personalEmail !== undefined ? personalEmail || null : undefined,
          phoneNumber:
            phoneNumber !== undefined ? phoneNumber || null : undefined,
  ...
  async (req: any, res: Response) => {
    try {
      ...
      await (prisma as any).$transaction([
        (prisma as any).agentMarginSegment.deleteMany({
          where: { agentId: id },
        }),
        ...(segments.length > 0
          ? [
              (prisma as any).agentMarginSegment.createMany({
                data: segments.map((s: any) => ({
                  agentId: id,
                  minAmount: parseFloat(s.minAmount),
                  maxAmount:
                    s.maxAmount !== null && s.maxAmount !== undefined
      ...
      res
        .status(200)
        .json({ message: "Margin segments updated successfully", agent });
    } catch (error: any) {
...
app.get(
  ...
  async (req: any, res: Response) => {
    try {
      ...
      const wallet = await (prisma as any).agentWallet.findUnique({
        where: { agentId },
      });
  ...
  async (req: any, res: Response) => {
    try {
      ...
      if (!vendor)
        return res
          .status(404)
          .json({ error: "Not Found", message: "Vendor not found" });
...
app.post("/vendors", requireTenantContext, async (req: any, res: Response) => {
  try {
    const tenantId = parseInt(req.headers["x-tenant-id"] as string);
    const { name, phoneNumber, email, website, vendorType, creditBalance } =
      req.body;
    if (!name)
      return res
        .status(400)
    ...
    try {
      ...
      const vendor = await (prisma as any).vendor.update({
        ...
        data: {
          ...
          creditBalance:
            creditBalance !== undefined ? creditBalance : undefined,
        },
      });
...
async function getSmtpTransporter() {
  const hostSetting = await prisma.systemSetting.findUnique({
    where: { key: "smtp_host" },
  });
  const portSetting = await prisma.systemSetting.findUnique({
    where: { key: "smtp_port" },
  });
  const secureSetting = await prisma.systemSetting.findUnique({
  ...
  console.log(
    `Creating SMTP transporter: ${host}:${port} (secure: ${secure}) with user ${user}`,
  );
  ...
  return new Promise((resolve, reject) => {
    try {
      ...
      doc
        ...
        .font("Helvetica-Bold")
        .fontSize(16)
        .text(data.companyName.toUpperCase(), 40, headerY);
      doc
        .fillColor("#64748b")
        .font("Helvetica")
        .fontSize(9)
        .text("Official Salary Slip", 40, headerY + 18);
      ...
      doc
        ...
        .fontSize(10)
        .text(
          `Pay Period: ${data.periodFromStr} - ${data.periodToStr}`,
          300,
          headerY,
      ...
      doc
        ...
        .font("Helvetica")
        .fontSize(9)
        .text(`Issued: ${data.issueDateStr}`, 300, headerY + 15, {
          align: "right",
          width: 255,
        });
      ...
      doc
        ...
        .fontSize(8)
        .font("Helvetica-Bold")
        .text("EMPLOYEE / AGENT", 55, currentY + 10);
      doc
        .fillColor("#0f172a")
        .fontSize(11)
        .font("Helvetica-Bold")
        .text(data.agentName, 55, currentY + 22);
      doc
        .fillColor("#475569")
        .fontSize(9)
        .font("Helvetica")
        .text(data.agentEmail, 55, currentY + 36);

      doc
        .fillColor("#64748b")
        .fontSize(8)
        .font("Helvetica-Bold")
        .text("GDS SYSTEM / PCC", 330, currentY + 10);
      doc
        .fillColor("#0f172a")
        .fontSize(11)
        .font("Helvetica-Bold")
        .text(`${data.gdsSystem} / ${data.pcc}`, 330, currentY + 22);
      currentY += 80;
      ...
      doc
        ...
        .fontSize(13)
        .font("Helvetica-Bold")
        .text(String(data.totalWorkdays), 40, currentY + 10, {
          width: 171,
          align: "center",
        });
      doc
        ...
        .fontSize(8)
        .font("Helvetica-Bold")
        .text("TOTAL WORKDAYS", 40, currentY + 26, {
          width: 171,
          align: "center",
        });
      ...
      doc
        ...
        .fontSize(13)
        .font("Helvetica-Bold")
        .text(String(data.daysPresent), 211, currentY + 10, {
          width: 171,
          align: "center",
        });
      doc
        ...
        .fontSize(8)
        .font("Helvetica-Bold")
        .text("DAYS PRESENT", 211, currentY + 26, {
          width: 171,
          align: "center",
        });
      ...
      doc
        ...
        .fontSize(13)
        .font("Helvetica-Bold")
        .text(String(data.absents), 382, currentY + 10, {
          width: 173,
          align: "center",
        });
      doc
        ...
        .fontSize(8)
        .font("Helvetica-Bold")
        .text("DAYS ABSENT", 382, currentY + 26, {
          width: 173,
          align: "center",
        });
      ...
      const drawRow = (
        ...
        if (earn) {
          doc
            .fillColor(earn.startsWith("+") ? "#16a34a" : "#0f172a")
            .font("Helvetica-Bold")
            .text(earn, 340, currentY + 6, { width: 100, align: "right" });
        } else {
          doc
            .fillColor("#cbd5e1")
            .text("--", 340, currentY + 6, { width: 100, align: "right" });
        }
        ...
        if (ded) {
          doc
            .fillColor("#dc2626")
            .font("Helvetica-Bold")
            .text(ded, 440, currentY + 6, { width: 100, align: "right" });
        } else {
          doc
            .fillColor("#cbd5e1")
            .text("--", 440, currentY + 6, { width: 100, align: "right" });
        }
        currentY += 22;
      };

      const dailyRate =
        data.totalWorkdays > 0 ? data.basicSalaryVal / data.totalWorkdays : 0;
      const absentDeduction = dailyRate * data.absents;
      const totalAllowancesVal = data.allowances.reduce(
        (sum, a) => sum + Number(a.amount),
        0,
      );
      const holidayPay =
        data.paidHolidaysCount * data.paidHolidaysRate +
        data.publicHolidaysCount * data.publicHolidaysRate;
      const grossEarnings =
        data.basicSalaryVal +
        data.marginEarnedVal +
        totalAllowancesVal +
        holidayPay;
      const totalDeductions =
        absentDeduction +
        data.deductions.reduce((sum, d) => sum + Number(d.amount), 0);
      const finalNetPay = Math.max(0, grossEarnings - totalDeductions);

      const curSym = data.currencySymbol || "£";

      drawRow(
        "Basic Contract Salary (Monthly contract)",
        `${curSym}${data.basicSalaryVal.toFixed(2)}`,
        "",
      );
      drawRow(
        "Booking Commission / Margin Share",
        `+${curSym}${data.marginEarnedVal.toFixed(2)}`,
        "",
      );
      ...
      if (data.paidHolidaysCount > 0) {
        drawRow(
          `Paid Holidays (${data.paidHolidaysCount} days @ ${curSym}${data.paidHolidaysRate.toFixed(2)}/day)`,
          `+${curSym}${(data.paidHolidaysCount * data.paidHolidaysRate).toFixed(2)}`,
          "",
        );
      }
      ...
      if (data.publicHolidaysCount > 0) {
        drawRow(
          `Public Holidays (${data.publicHolidaysCount} days @ ${curSym}${data.publicHolidaysRate.toFixed(2)}/day)`,
          `+${curSym}${(data.publicHolidaysCount * data.publicHolidaysRate).toFixed(2)}`,
          "",
        );
      }
      ...
      if (data.absents > 0 && dailyRate > 0) {
        drawRow(
          `Absenteeism Penalty (${data.absents} days absent @ ${curSym}${dailyRate.toFixed(2)}/day)`,
          "",
          `-${curSym}${absentDeduction.toFixed(2)}`,
        );
      }
      ...
      doc
        ...
        .fontSize(8)
        .font("Helvetica-Bold")
        .text("SUBTOTALS", 50, currentY + 6);
      doc
        .fillColor("#0f172a")
        .text(`${curSym}${grossEarnings.toFixed(2)}`, 340, currentY + 6, {
          width: 100,
          align: "right",
        });
      doc
        .fillColor("#dc2626")
        .text(`${curSym}${totalDeductions.toFixed(2)}`, 440, currentY + 6, {
          width: 100,
          align: "right",
        });
      currentY += 20;
      ...
      doc
        ...
        .fontSize(10)
        .font("Helvetica-Bold")
        .text("TOTAL NET PAYABLE (NET SALARY)", 50, currentY + 9);
      doc
        .fillColor("#38bdf8")
        .fontSize(12)
        .font("Helvetica-Bold")
        .text(`${curSym}${finalNetPay.toFixed(2)}`, 340, currentY + 8, {
          width: 200,
          align: "right",
        });
      currentY += 28;
      ...
      if (data.notes) {
        ...
        doc
          ...
          .fontSize(8)
          .font("Helvetica-Bold")
          .text("NOTES & REMARKS", 50, currentY + 6);
        doc
          .fillColor("#78350f")
          .fontSize(8)
          .font("Helvetica")
          .text(data.notes, 50, currentY + 18, { width: 495 });
        currentY += 45;
      }
      ...
      doc
        ...
        .font("Helvetica")
        .text(
          `This is a secure official statement generated by your employer (${data.companyName}).`,
          40,
          currentY + 10,
          { align: "center", width: 515 },
        );
      doc.text(
        "If you have any inquiries regarding this document, please contact the finance team.",
        40,
        currentY + 20,
        { align: "center", width: 515 },
      );
...
async function getTenantSmtpTransporter(
  tenantId: number,
): Promise<{
  transporter: any;
  fromEmail: string;
  ...
  return {
    transporter,
    fromEmail,
    fromName: tenant?.name || "Techbarred",
    tenant,
  };
}
...
app.post("/tenants/send-email", async (req: Request, res: Response) => {
  try {
    ...
    const { transporter, fromEmail, fromName, tenant } =
      await getTenantSmtpTransporter(tenantId);
  ...
  try {
    ...
    if (!fullName || !email || !companyName) {
      return res
        ...
        .json({
          error: "Validation failed",
          message: "Full Name, Work Email, and Company Name are required.",
        });
    }
...
app.get(
  ...
  async (req: any, res: Response) => {
    try {
      const host = await prisma.systemSetting.findUnique({
        where: { key: "smtp_host" },
      });
      const port = await prisma.systemSetting.findUnique({
        where: { key: "smtp_port" },
      });
      const secure = await prisma.systemSetting.findUnique({
        where: { key: "smtp_secure" },
      });
      const user = await prisma.systemSetting.findUnique({
        where: { key: "smtp_user" },
      });
      const pass = await prisma.systemSetting.findUnique({
        where: { key: "smtp_pass" },
      });
    ...
    try {
      ...
      if (!host || !port || !user || !pass) {
        return res
          ...
          .json({
            error: "Validation failed",
            message: "Host, port, user, and password are required.",
          });
      }
...
app.patch(
  ...
  async (req: any, res: Response) => {
    try {
      ...
      res
        .status(200)
        .json({ message: "Demo request updated successfully", request });
    } catch (error) {
...
async function seedDefaultRolePermissions(roleId: number, roleName: string) {
  try {
    ...
    console.error(
      `Error seeding default permissions for role ${roleName}:`,
      error,
    );
  }
}
...
function getPermissionsForAccessLevel(
  module: string,
  accessLevel: string,
): string[] {
  if (module === "Agency Dashboard") {
  ...
  async (req: any, res: Response) => {
    try {
      ...
      for (const role of roles) {
        rolePermissionsMap[role.name] = role.permissions.map(
  ...
  async (req: any, res: Response) => {
    try {
      ...
      if (!permissions || typeof permissions !== "object") {
        return res
          .status(400)
          .json({ error: "Invalid payload: permissions map required" });
      }
      ...
      res
        .status(200)
        .json({ message: "Permissions matrix updated successfully" });
    } catch (error) {
  ...
  try {
    ...
    if (userRole === "AGENT") {
      if (!userId) {
        return res
          .status(200)
          .json({ allowed: false, message: "Missing user context" });
      }
      const user = await prisma.user.findUnique({
        where: { id: parseInt(userId as string) },
      });
    ...
    if (!permission) {
      return res
        .status(200)
        .json({ allowed: false, message: "Permission not found" });
    }
  ...
  async (req: any, res: Response) => {
    try {
      ...
      if (isNaN(userId)) {
        return res
          .status(401)
          .json({ error: "Unauthorized", message: "User context required" });
      }
      ...
      if (isPlatformAdmin) {
        ...
        return res
          .status(200)
          .json({ permissions: allPerms.map((p: any) => p.name) });
      }
